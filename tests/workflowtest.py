...
class Konvertierungsauftrag:
    def __init__(self, **kwargs): self.auftragId = 101; self.__dict__.update(kwargs)
class Textbeitrag:
    def __init__(self, **kwargs): self.textId = 201; self.__dict__.update(kwargs)
class Podcast:
    def __init__(self, **kwargs): self.podcastId = 301; self.__dict__.update(kwargs)
...
# ***********************************************************************************
    def run_pipeline(
        ...
        try:
            ...
            if zweitstimme:
                ...
                if secondary_voice_obj is None:
                    logging.warning(f"Zweitstimme '{zweitstimme}' nicht gefunden. Nur Hauptstimme wird verwendet.")
        ...
        config = {
            ...
            "speakers": speakers, "roles": roles if roles is not None else {}, "thema": thema
        }
        ...
        session.commit()
        logging.info(f"Metadaten gespeichert. Auftrag ID: {konvertierungsauftrag.auftragId} im Status IN_BEARBEITUNG.")
        ...
        try:
            ...
            session.commit()
            logging.info(f"Podcast mit ID {podcast.podcastId} erfolgreich abgeschlossen.")
            ...
            logging.error(f"Pipeline-Fehler, Auftrag ID {konvertierungsauftrag.auftragId} auf FEHLGESCHLAGEN gesetzt: {e}")
...
def run_test():
    ...
    TEST_PROMPT_FAIL = "Dieser Text enthält FEHLER und löst einen simulierten TTS-Fehler aus."
    try:
