...
def main_full_api_test():
    ...
    with patch("services.workflow.get_db") as MockGetDB, patch(
        "services.tts_service.get_db"
    ) as MockTTSGetDB, patch("services.tts_service.VoiceRepo") as MockTTSVoiceRepo:
        ...
        MockTTSVoiceRepo.return_value.get_voices_by_names.return_value = (
            []
        )  # Wir nutzen Hardcoding im Workflow
        ...
        try:
            ...
            logger.info(
                f"✅ Workflow erfolgreich. Audio generiert und gespeichert als: {audio_path}"
            )

        except Exception as e:
            logger.error(
                f"❌ Der Haupt-Workflow ist fehlgeschlagen: {e}", exc_info=True
            )
