...
def mock_controller():
    ...
    p2 = patch(
        "frontend.controller.get_podcasts_for_user", side_effect=backend.get_podcasts
    )
    p3 = patch(
        "frontend.controller.request_login_code", return_value=(True, "Code sent!")
    )
    p4 = patch(
        "frontend.controller.verify_login_code",
        return_value=(True, {"id": 1, "email": "test@smail.th-koeln.de"}, "Success"),
    )
    p5 = patch(
        "frontend.controller.generate_script", return_value="This is a mock script."
    )
    p6 = patch("frontend.controller.generate_audio_only", return_value="mock_audio_obj")
    p7 = patch(
        "frontend.controller.save_generated_podcast", side_effect=backend.save_podcast
    )
    p8 = patch(
        "frontend.controller.process_source_input", side_effect=backend.process_source
    )
